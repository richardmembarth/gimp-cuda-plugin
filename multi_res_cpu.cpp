...
template <typename T> float decompose(T *l0, T *g1, T *g0, const int width, const int height, unsigned int timer);
template <typename T> float filter(T *f0, T *l0, const int width, const int height, int sigma_d, int sigma_r, unsigned int timer);
template <typename T> float reconstruct(T *r0, T *r1, T *f0, const int width, const int height,unsigned int timer);
...
template <typename T> void bilateral_filter(T *reference, T *l0, const int width, const int height, int sigma_d, int sigma_r) {
    ...
    for (xf=-2*sigma_d; xf<=2*sigma_d; xf++) gaussian[xf+2*sigma_d] = expf(-c_d*(xf*xf));
    for (xf=-2*sigma_d; xf<=2*sigma_d; xf++) {
...
#ifdef PRINT_TIMES
    fprintf(stderr, "decompose time(%d:%d) CPU: %f (ms)\n", width, height, cutGetTimerValue(timer));
...
#ifdef PRINT_TIMES
    fprintf(stderr, "filter time(%d:%d) CPU: %f (ms)\n", width, height, cutGetTimerValue(timer));
    return cutGetTimerValue(timer);
#else
    return 0.0f;
#endif
...
#ifdef PRINT_TIMES
    fprintf(stderr, "reconstruct time(%d:%d) CPU: %f (ms)\n", width, height, cutGetTimerValue(timer));
    return cutGetTimerValue(timer);
#else
    return 0.0f;
#endif
...
template <typename T> void run_cpu_intern(guchar *host_g0, guchar *host_r0, int width, int height, int channels, int sigma_d, int sigma_r) {
    ...
    for (int i=0; i < channels; i++) {
        ...
        // decompose image
        time += decompose(l0, g1, g0, data_width, data_height, timer);
        time += decompose(l1, g2, g1, data_width/2, data_height/2, timer);
        time += decompose(l2, g3, g2, data_width/4, data_height/4, timer);
        time += decompose(l3, g4, g3, data_width/8, data_height/8, timer);
        time += decompose(l4, l5, g4, data_width/16, data_height/16, timer);

        // filter image - reuse g0 for f0
        time += filter(g0, l0, data_width, data_height, sigma_d, sigma_r, timer);
        time += filter(g1, l1, data_width/2, data_height/2, sigma_d, sigma_r, timer);
        time += filter(g2, l2, data_width/4, data_height/4, sigma_d, sigma_r, timer);
        time += filter(g3, l3, data_width/8, data_height/8, sigma_d, sigma_r, timer);
        time += filter(g4, l4, data_width/16, data_height/16, sigma_d, sigma_r, timer);
        time += filter(g5, l5, data_width/32, data_height/32, sigma_d, sigma_r, timer);

        // reconstruct image - reuse l0 for r0
        time += reconstruct(l4, g5, g4, data_width/32, data_height/32, timer);
        time += reconstruct(l3, l4, g3, data_width/16, data_height/16, timer);
        time += reconstruct(l2, l3, g2, data_width/8, data_height/8, timer);
        time += reconstruct(l1, l2, g1, data_width/4, data_height/4, timer);
        time += reconstruct(l0, l1, g0, data_width/2, data_height/2, timer);
